...
extends		AbstractCVM
{
	...
	protected static double		TARGET_FREEZING_TEMPERATURE = -15.0;
	protected static double		TARGET_REFRIGERATION_TEMPERATURE = 4.0;
	...
	public void			deploy() throws Exception
	{
		...
		// create the the two components implementing the refrigerator
		// i.e., the refrigerator and its controller
		AbstractComponent.createComponent(
				Refrigerator.class.getCanonicalName(),
				new Object[]{IS_UNDER_TEST, IS_UNIT_TESTING, IS_SIMULATED,
							 UnitTestSupervisor.SIM_ARCHITECTURE_URI,
							 ACC_FACTOR, CLOCK_URI});
		AbstractComponent.createComponent(
				RefrigeratorController.class.getCanonicalName(),
				new Object[]{
						Refrigerator.SENSOR_INBOUND_PORT_URI,
						Refrigerator.ACTUATOR_INBOUND_PORT_URI,
						RefrigeratorController.STANDARD_HYSTERESIS,
						RefrigeratorController.STANDARD_CONTROL_PERIOD,
						IS_UNIT_TESTING, CLOCK_URI});

		// create a user component that will switch on the refrigerator, set its
		// target temperature and then switch it off just before the end of the
		// execution
		AbstractComponent.createComponent(
				RefrigeratorUser.class.getCanonicalName(),
				new Object[]{Refrigerator.INBOUND_PORT_URI,
							 EXECUTION_DURATION, 
							 TARGET_FREEZING_TEMPERATURE,
							 TARGET_REFRIGERATION_TEMPERATURE,
							 CLOCK_URI});

		// if SIL simulated, a supervisor component is required to start the
		// simulation (no coordinator component is needed because only one
		// component (the  refrigerator) has a SIL simulator
		if (IS_SIMULATED) {
			AbstractComponent.createComponent(
